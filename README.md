...
**LegalIR-PCSR** is an automated Information Retrieval (IR) system for legal statutes and prior case retrieval. It replicates and extends the IL-PCSR benchmark (Paul et al., EMNLP 2025) by implementing multiple retrieval methods and a citizen-facing RAG pipeline.
...
User Query (plain English)
    |
    +-- Ensemble → top-3 statutes  --+
    |                                    |
    +-- Ensemble → top-3 precedents -+
                                         |
                                         v
...
> 1. Retrieves top-3 relevant statutes using Ensemble
> 2. Retrieves top-3 relevant precedents using Ensemble
